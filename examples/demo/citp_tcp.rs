    ...
    pub fn read_message(&mut self) -> io::Result<()> {
        ...
        loop {
            ...
            match &header.content_type.to_le_bytes() {
                pinf::Header::CONTENT_TYPE => {
                    ...
                    match &message_content_type {
                        pinf::PNam::CONTENT_TYPE => {
                            let pnam = pinf::PNam::read_from_bytes(&received[read_offset..]).unwrap();
                            //println!("pnam = {:#?}", pnam);
                            message_size = pnam.size_bytes();
                        }
                        pinf::PLoc::CONTENT_TYPE => {
                            let ploc = pinf::PLoc::read_from_bytes(&received[read_offset..]).unwrap();
                            //println!("ploc = {:#?}", ploc);
                ...
                caex::Header::CONTENT_TYPE => {
                    match layer_two_content_type(&received, header_size) {
                        ...
                        caex::LaserFeedControl::CONTENT_TYPE => {
                            let feed_control = caex::LaserFeedControl::read_from_bytes(&received[read_offset..]).unwrap();
                        ...
                        caex::EnterShow::CONTENT_TYPE => {
                            println!("EnterShow");
                            let enter_show = caex::EnterShow::read_from_bytes(&received[read_offset..]).unwrap();
                ...
                _ => {
                    println!("Un recognized TCP Header Content Type {}", header.content_type);
                    break;
                },
            }
